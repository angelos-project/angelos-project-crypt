...
internal open class MutableBigInteger {
    ...
    fun rightShift(n: Int) {
        ...
        val bitsInHighWord = 32 - Integer.numberOfLeadingZeros(value[offset])
    ...
    private fun getLower(n: Int): BigInteger {
        return if (isZero) {
            ...
            BigInteger(Arrays.copyOfRange(value, offset + intLen - len, offset + intLen).contentToString(), sign)
        }
    }
    ...
    fun addDisjoint(addend: MutableBigInteger, n: Int) {
        ...
        y -= x
        rstart -= x
        val len = min(y.toDouble(), (addend.value.size - addend.offset).toDouble()).toInt()
    ...
    private fun difference(b: MutableBigInteger?): Int {
        ...
        while (y > 0) {
            ...
            diff = (a.value[a.offset + x].toLong() and LONG_MASK) -
                    (b.value[b.offset + y].toLong() and LONG_MASK) + (diff shr 32)
            a.value[a.offset + x] = diff.toInt()
        }
        // Subtract remainder of longer number
        while (diff < 0 && x > 0) {
            x--
            diff = (a.value[a.offset + x].toLong() and LONG_MASK) + (diff shr 32)
            a.value[a.offset + x] = diff.toInt()
        }
        a.normalize()
        return sign
    ...
    fun divideKnuth(
        ...
        if (intLen >= KNUTH_POW2_THRESH_LEN) {
            val trailingZeroBits = min(lowestSetBit.toDouble(), b.lowestSetBit.toDouble()).toInt()
    ...
    fun divide(v: Long, quotient: MutableBigInteger): Long {
        ...
        return if (d == 0) divideOneWord(
            v.toInt(),
            quotient
        ).toLong() and LONG_MASK else {
    ...
    private fun divideMagnitude(
        ...
        if (qhat != 0) {
            ...
            borrow =
                if (needRemainder) mulsub(rem.value, divisor, qhat, dlen, limit - 1 + rem.offset) else mulsubBorrow(
                    rem.value,
                    divisor,
                    qhat,
                    dlen,
    ...
    fun hybridGCD(b: MutableBigInteger?): MutableBigInteger? {
        ...
        while (b!!.intLen != 0) {
            if (abs((a!!.intLen - b.intLen).toDouble()) < 2) return a.binaryGCD(b)
            val r = a.divide(b, q)
            a = b
            b = r
        }
     ...
     */
    private fun binaryGCD(v_: MutableBigInteger): MutableBigInteger {
    ...
    private fun modInverse(mod: MutableBigInteger): MutableBigInteger? {
        ...
        var f: MutableBigInteger? = MutableBigInteger(this)
        var g: MutableBigInteger? = MutableBigInteger(p)
    ...
    fun euclidModInverse(k: Int): MutableBigInteger {
        ...
        var a: MutableBigInteger? = MutableBigInteger(this)
        var q: MutableBigInteger? = MutableBigInteger()
        var r = b.divide(a, q)
        var swapper = b
        // swap b & r
        b = r
